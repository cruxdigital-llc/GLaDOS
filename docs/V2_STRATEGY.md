...
- *`glados install` vendors the CI check templates into `.glados/ci/` and prints the enable stanza, and `glados doctor` verifies the check actually runs* — the project owns enabling it (adding the GitLab include or copying the GitHub workflow, per the boundary of responsibility, decision 9), but a check that's declared and never running is a reader without a writer, so its absence is loudly reported rather than silently assumed. Every check rule ships `report-only` first, promoted per-rule to `blocking` in the manifest: teams tighten what has proven quiet, mirroring the tier philosophy.
...
|---|---|---|
| **Claude Code** (plugin) | `skills/<core>/SKILL.md` wrappers, as today. The checked-in plugin artifacts compile from `glados.yaml.example` — a phase-neutral optimize-for plus a compiled bootstrap guard directing repos with a manifest of their own to a real install | Stop-hook (blocks session end) |
...
Installer→assembler (inlining, vocabulary partials, compiled epilogue, manifest hash); minimal `glados.yaml` (platform, three channel bindings — `verdict: mr-comment`, `escalation|bug: issue`, `progress: repo` — per-workflow module lists, a `default-modules:` list for unlisted workflows — phase presets supply it when not explicit — assembly report printed at install); flat state-key registry + set-membership check; run ledger with the two-strata rule; merge authority as a manifest key; base-SHA yield rule; Claude Code Stop-hook; CI templates vendored into `.glados/ci/` by the installer, which prints the enable stanza — the project adds the include; rules report-only first; the full consolidation and delete list; MIGRATION.md (copy-the-example + hand-edit; `install` prints a prefilled-manifest hint when it detects v1 leftovers — no `glados migrate` machinery); permanent alias shims. **Plus first-class SDA conformance** via one explicit-only manifest key (`sda: true` — a team declaration; no phase preset may set it): install-time create-only scaffolding (claims.md, SPEC_LOG.md, version headers, standards docs) and lane-2 conditional claim / work-unit-log steps in the compiled preamble/epilogue, with the v2 mapping documented in a new versioned profile (docs/standards/sda-profile-glados-v2.md — the run ledger declared the work-unit equivalent of the retired `specs/` dirs).
**Plus the phase sliver** (costs days, and makes the plumbing release adoptable on greenfield repos without production-grade ceremony): required `phase:` key, the four presets expanding over v2.0.0 keys only, provenance/`RELAXED` reporting, decision keys + the `decision` outcome type, the `optimize-for` preamble stanza, CODEOWNERS guidance for `glados.yaml` (the MIGRATION snippet; `doctor` reports coverage informationally), *advisory* transition checklists printed by `install` when the declared phase changes.
